...
bool Consensus::CheckTxInputs(const CTransaction& tx, TxValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, CAmount& txfee, const Consensus::Params& params, unsigned int nTimeTx, uint64_t nMoneySupply)
{
    ...
    if (tx.IsCoinStake())
    {
        // nowp: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
        ...
        //return the blockreward on the txfee field, this will be used later to validate the reward amount
        txfee = tx.GetValueOut() - nValueIn;
    }
    else
    {
        ...
        // nowp: enforce transaction fees for every block
