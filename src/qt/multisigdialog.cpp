...
void MultisigDialog::on_saveMultisigAddressButton_clicked()
{
    ...
    if(!wallet->mapAddressBook.count(DecodeDestination(address)))
        wallet->SetAddressBook(DecodeDestination(address), label, "");  //nowpTODO - maybe add purpose instead of empty string?
}
...
void MultisigDialog::on_signTransactionButton_clicked()
{
    ...
    //nowpTODO - not sure how to fix this properly
    // Sign what we can
    bool fComplete = true;
...
void MultisigDialog::on_sendTransactionButton_clicked()
{
    ...
    ui->statusLabel->setText(tr("The transaction is sent to nowp network."));
}
