...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    // nowp: if transaction is after version 0.8 fork, verify SCRIPT_VERIFY_LOW_S
    // nowpTODO move back to policy.h after 0.8 is active
...
int64_t GetBlockReward(int height, const Consensus::Params &params)
{

    int64_t nSubsidy = 188 * COIN;
    if(height == 1) {
        nSubsidy = 12600000 * COIN;
    } else if(height <= 1440) {
        nSubsidy = 10 * COIN;
    } else {
        int halvings = height / params.nSubsidyHalvingInterval;
        if (halvings >= 128)
            return 0;
...
{
    if (fUpdateUI)
        uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED); // nowp: we are using arguments that will have no effects in updateAlert()
#if HAVE_SYSTEM
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // nowp
            undo.nTime = alternate.nTime;           // nowp
        } else {
...
bool NowpContextualBlockChecks(const CBlock& block, BlockValidationState& state, CBlockIndex* pindex, bool fJustCheck, CChainState& chainstate)
{
    uint256 hashProofOfStake = uint256();
    // nowp: verify hash target and signature of coinstake tx
    ...
    // nowp: check for duplicity of stake
    ...
    // nowp: compute stake entropy bit for stake modifier
    unsigned int nEntropyBit = GetStakeEntropyBit(block);

    // nowp: compute stake modifier
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (pindex->nStakeModifier == 0 && pindex->nStakeModifierChecksum == 0 && !NowpContextualBlockChecks(block, state, pindex, fJustCheck, m_chainman.ActiveChainstate()))
    ...
    if (block.vtx[0]->GetValueOut() > (block.IsProofOfWork() ? blockReward : 0))
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-amount",
                             strprintf("CheckBlock() : coinbase reward exceeded %s > %s",
                                       ...
                                       FormatMoney(block.IsProofOfWork() ? blockReward : 0)));
    ...
    // nowp: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;

    // nowp: fees are not collected by miners as in bitcoin
    // nowp: fees are destroyed to compensate the entire network
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature)
{
    ...
    // nowp: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinStake())
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cs-missing", "coinstake in wrong position");

    // nowp: first coinbase output should be empty if proof-of-stake block
    ...
    if (block.GetBlockTime() > (block.vtx[0]->nTime ? (int64_t)block.vtx[0]->nTime : block.GetBlockTime()) + MAX_FUTURE_BLOCK_TIME)
    ...
    for (const auto& tx : block.vtx) {
        ...
        if (!CheckTransaction(*tx, tx_state)) {
            ...
            // nowp: check transaction timestamp
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    // nowp: we should only accept blocks that can be connected to a prev block with validated PoS
    ...
    // nowp: check PoS
    if (fCheckPoS && !NowpContextualBlockChecks(block, state, pindex, false, m_chainman.ActiveChainstate())) {
        pindex->nStatus |= BLOCK_FAILED_VALID;
        m_blockman.m_dirty_blockindex.insert(pindex);
...
// nowp: total coin age spent in transaction, in the unit of coin-days.
