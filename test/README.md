This directory contains integration tests that test nowpd and its
...
nowpd and bitcoin-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (nowp-util, nowp-tx, ...).
...
The P2P and RPC ports used by the nowpd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another nowpd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its nowpd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other nowpd processes are running.

On linux, the test framework will warn if there is another
nowpd process running when the tests are started.

If there are zombie nowpd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
nowpd processes running on the system, so should not be used if any non-test
nowpd processes are being run.**
...
sure nowpd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and nowpd `debug.log`s will all be dumped
...
`test_framework.log` and nowpd `debug.log`s can be combined into a single
...
call methods that interact with the nowpd nodes-under-test.

If further introspection of the nowpd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/nowpd.pid
```
