...
The data directory is the default location where the Nowp files are stored.
...
---------|--------------------
Linux    | `$HOME/.nowp/`
macOS    | `$HOME/Library/Application Support/Nowp/`
Windows  | `%APPDATA%\Nowp\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `nowp.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Nowp blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `nowp.conf`       | Contains [configuration settings](nowp-conf.md) for `nowpd` or `nowp-qt`; File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `nowpd.pid`       | Stores the process ID (PID) of `nowpd` or `nowp-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `nowpd` or `nowp-qt`; can be specified by `-debuglogfile` option
...
`nowp-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Nowp:
