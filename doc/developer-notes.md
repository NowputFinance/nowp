...
Run with the `-testnet` option to run with "play nowps" on the test network, if you
...
Nowp is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_nowp
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_nowp --log_level=test_suite
...
# A coverage report will now be accessible at `./test_nowp.coverage/index.html`.
```
...
However, in open source software such as Nowp, where everyone uses
...
pay attention to for reviewers of Nowp code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Nowp uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Nowp, in which case
...
a Nowp subtree, so that they can be integrated quickly. Cosmetic changes
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `nowp-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Nowp, the wallet was always
